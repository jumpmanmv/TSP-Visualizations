...
void init()
{
    ...
    gWin = SDL_CreateWindow("ACO TSP", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
    ...
    loadNewText(&gTitle, "ACO TSP Visualization", 1);
...
void connectCities(city cities[], int **memory, int bestAnt)
{
    SDL_SetRenderDrawColor(gRend, 0x90, 0xCA, 0xF9, 0xFF);
    for (int i = 0; i < NUM_OF_CITIES-1; i++)
    {
        SDL_RenderDrawLine(gRend, (CITY_SIZE*cities[memory[bestAnt][i]].x)+(CITY_SIZE/2), (CITY_SIZE*cities[memory[bestAnt][i]].y)+(CITY_SIZE/2)+TOP_PADDING,
        (CITY_SIZE*cities[memory[bestAnt][i+1]].x)+(CITY_SIZE/2), (CITY_SIZE*cities[memory[bestAnt][i+1]].y)+(CITY_SIZE/2)+TOP_PADDING);
        SDL_RenderPresent(gRend);
    }
}

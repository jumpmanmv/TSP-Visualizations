...
void init()
{
    ...
    gWin = SDL_CreateWindow("Greedy TSP", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
    ...
    loadNewText(&gTitle, "Greedy TSP Visualization", 1);
    loadNewText(&gCredits, "Created by: George Vlassis & Manos Vlassis", 0);
    loadNewText(&gIterationText, "Calculating element: ", 0);
}
...
void connectCities(city cities[], int path[])
{
    SDL_SetRenderDrawColor(gRend, 0x90, 0xCA, 0xF9, 0xFF);
    for (int i = 0; i < NUM_OF_CITIES-1; i++)
    {
        SDL_RenderDrawLine(gRend, (CITY_SIZE*cities[path[i]].x)+(CITY_SIZE/2), (CITY_SIZE*cities[path[i]].y)+(CITY_SIZE/2)+TOP_PADDING,
        (CITY_SIZE*cities[path[i+1]].x)+(CITY_SIZE/2), (CITY_SIZE*cities[path[i+1]].y)+(CITY_SIZE/2)+TOP_PADDING);
        SDL_RenderPresent(gRend);
    }
}
...
void loadNewText(text *t, const char* str, int isTitleFont)
{
    ...
    {
        loadedSurface = TTF_RenderText_Blended_Wrapped(gTitleFont, str, textColor, SCREEN_WIDTH);  
    }
